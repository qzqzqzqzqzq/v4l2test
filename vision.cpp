...
void vision_loop()
{
	...
	while(!allfinishflag.load())
	{
        ...
        cap.read(frame);// 因为我们在循环外读了第一帧，所以最后再读下一帧
	}
    // ================== 新增：释放 writer ==================
    std::cout << "[INFO] 视觉线程结束，释放 VideoWriter..." << std::endl;
    writer.release();
    cap.release();
}
...
void dect_rxd(const cv::Mat& cropped_image){
    ...
    //矩形筛选：1、面积处于一定范围；2、宽度大于某个值；3、数量大于某个值；满足以上判断为此帧发现斑马线
